    ...
    case 2: // Delay (Slot 5)
      if (subId == 0) {
        if (mDelaySync) {
          ...
          int idx = (int)(value * 19.99f);
          float div = 0.25f;
          switch (idx) {
            case 0: div = 0.125f; break;     // 1/32
            case 1: div = 0.16667f; break;   // 1/24
            case 2: div = 0.2f; break;       // 1/5
            case 3: div = 0.25f; break;      // 1/16
            case 4: div = 0.33333f; break;   // 1/12
            case 5: div = 0.375f; break;     // 1/16D
            case 6: div = 0.5f; break;       // 1/8
            case 7: div = 0.66667f; break;   // 1/4T
            case 8: div = 0.75f; break;      // 1/8D
            case 9: div = 0.8f; break;       // 4/5
            case 10: div = 1.0f; break;      // 1/4
            case 11: div = 1.33333f; break;  // 1/2T
            case 12: div = 1.33333f; break;  // 4/3
            case 13: div = 1.5f; break;      // 3/8
            case 14: div = 2.0f; break;      // 1/2
            case 15: div = 2.5f; break;      // 5/8
            case 16: div = 3.5f; break;      // 7/8
            case 17: div = 4.0f; break;      // 1/1
            case 18: div = 5.0f; break;      // 5/4
            case 19: div = 6.0f; break;      // 3/2
          }
        ...
        float beatLen = 60.0f / std::max(1.0f, mBpm);
        if (mDelaySync) {
          int idx = (int)(currentVal * 19.99f);
          float div = 0.25f;
          switch (idx) {
            case 0: div = 0.125f; break;
            case 1: div = 0.16667f; break;
            case 2: div = 0.2f; break;
            case 3: div = 0.25f; break;
            case 4: div = 0.33333f; break;
            case 5: div = 0.375f; break;
            case 6: div = 0.5f; break;
            case 7: div = 0.66667f; break;
            case 8: div = 0.75f; break;
            case 9: div = 0.8f; break;
            case 10: div = 1.0f; break;
            case 11: div = 1.33333f; break;
            case 12: div = 1.33333f; break;
            case 13: div = 1.5f; break;
            case 14: div = 2.0f; break;
            case 15: div = 2.5f; break;
            case 16: div = 3.5f; break;
            case 17: div = 4.0f; break;
            case 18: div = 5.0f; break;
            case 19: div = 6.0f; break;
          }
          mDelayFx.setDelayTime(beatLen * div);
        } else {
...
void AudioEngine::renderInput(const float *inputData, int32_t numFrames, int32_t channels) {
  ...
  if (mIsRecordingSample.load() && mRecordingTrackIndex != -1 && (mRecordingSource == MIC || mRecordingSource == LINE_IN || mRecordingSource == MIX)) {
    std::lock_guard<std::recursive_mutex> lock(mLock);
  ...
  if (numFrames > 0) {
    ...
    for (int i = 0; i < numFrames; i += MAX_MELODY_BATCH) {
      ...
      for (int f = 0; f < framesToDo; ++f) {
        ...
        if (channels == 2) {
          if (mRecordingSource == MIC) {
            melodyBatch[f] = inputData[idx * 2];
          } else if (mRecordingSource == LINE_IN) {
            melodyBatch[f] = inputData[idx * 2 + 1];
          } else {
            melodyBatch[f] = (inputData[idx * 2] + inputData[idx * 2 + 1]) * 0.5f;
          }
        } else {
...
void AudioEngine::requestCapture(bool active) {
  if (active) {
    int prev = mCaptureUserCount.fetch_add(1);
    if (prev == 0 && mOnCaptureStateChanged) {
      mOnCaptureStateChanged(true);
    }
  } else {
    int cur = mCaptureUserCount.load();
    while (cur > 0) {
      if (mCaptureUserCount.compare_exchange_weak(cur, cur - 1)) {
        if (cur == 1 && mOnCaptureStateChanged) {
          mOnCaptureStateChanged(false);
        }
        break;
      }
    }
...
void pushLaunchkeyLedUpdate(AudioEngine* engine, UIManager* ui) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        // MK3 Drum pads in DAW mode map to notes 96-111 on Channel 16 (0x0F)
        snd_seq_ev_set_noteon(&ev, 15, 96 + i, color);
        snd_seq_event_output_direct(gSeqOut, &ev);
        
        // MK4 uses Channel 1 (0x00) with a split layout:
        // Top row (i=0..7) -> Notes 96..103
        // Bottom row (i=8..15) -> Notes 112..119
        int mk4Note = (i < 8) ? (96 + i) : (112 + (i - 8));
        snd_seq_ev_set_noteon(&ev, 0, mk4Note, color);
        snd_seq_event_output_direct(gSeqOut, &ev);
    }
#endif
}
